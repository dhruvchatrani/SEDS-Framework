...
from core.state import MAESState, StrategicCommunication, Proposal
...
def strategic_planning_node(state: MAESState):
...
def fiscal_validation_node(state: MAESState):
...
def risk_assessment_node(state: MAESState):
...
def executive_adjudication_node(state: MAESState):
...
def consensus_routing_logic(state: MAESState) -> Literal["strategic_planning_node", "__end__"]:
